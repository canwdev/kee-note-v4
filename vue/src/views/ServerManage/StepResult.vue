  ...
  setup(props, {emit}) {
    ...
    const handleSaveToServer = async () => {
      try {
        ...
        // get KN_HTTP_CRYPT_KEY from env text
        const cryptKey = mValue.value.match(/KN_HTTP_CRYPT_KEY=(.*)/)?.[1]

        window.$dialog.success({
          title: 'Saved!',
          content: message,
          positiveText: cryptKey ? 'Update crypt key and reload' : 'OK',
          onPositiveClick: async () => {
            if (cryptKey) {
              localStorage.setItem(LsKeys.LS_KEY_KN_HTTP_CRYPT_KEY, cryptKey)
