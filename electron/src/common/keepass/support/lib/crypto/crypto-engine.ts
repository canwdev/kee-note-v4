...
export function hmacSha256(key: ArrayBuffer, data: ArrayBuffer): Promise<ArrayBuffer> {
  if (global.crypto?.subtle) {
    ...
    return global.crypto.subtle.importKey('raw', key, algo, false, ['sign']).then((subtleKey) => {
...
export function argon2(
  ...
  if (argon2impl) {
    return argon2impl(password, salt, memory, iterations, length, parallelism, type, version).then(
      arrayToBuffer
    )
  }
